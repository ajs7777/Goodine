...
//  Goodine
//
//  Created by Abhijit Saha on 04/06/25.
//

import SwiftUI

struct UserHistoryDetailedView: View {
                        ...
                        VStack(alignment: .leading, spacing: 7) {
                            ...
                            ForEach(sortedKeys, id: \.self) { key in
                                if let item = order.items[key] {
                                    UserOrderRowHistory(item: item, orderId: order.id ?? "", reservationId: reservationId, orderVM: orderVM)
                                }
                            }
                        }
                        .padding(.vertical, 5)
    ...
    private func generatePDF(historyItem: HistoryRecord) {
        ...
        let pdfData = renderer.pdfData { context in
            ...
            yOffset += 10
            let restaurant = businessAuthVM.restaurant
            drawCenteredText("Platform Fee: \(restaurant?.currencySymbol ?? "₹")3.00", font: bodyFont)
...
struct UserOrderRowHistory: View {
    let item: OrderItem
    let orderId: String
    let reservationId: String
